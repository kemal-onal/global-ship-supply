...
class TestMovement:
    ...
    def test_high_scale_no_orbit(self) -> None:
        ...
        for _ in range(2):
            for r, _ in w.step():
                pass
        # Track COG over 20 sim-hours. The heading should be stable.
        cogs = []
        for _ in range(20):
            for r, _ in w.step():
                cogs.append(r.cog)
        ...
        for _ in range(200):
            for r, events in w.step():
                if any(e.event_type == "port_arrival" for e in events):
                    arrived = True
                    # Should be at Shanghai now.
                    assert r.destination_port_id == "SGSIN"  # next leg's destination
        assert arrived, "vessel never arrived in 200 sim-hours"
